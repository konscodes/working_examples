...
def MainFunction():
    try:
        ...
        StopDistance = round((SetRisk/int(CurrentPosition)), 2)
        ...
        # Send DAS hotkey
        PywinautoKeyboard.send_keys(DasHotkey)
        print("\nSending ", DasHotkey, " hotkey to DAS")
        PrintMessage()
...
StartHotkey = "<Ctrl>+<Alt>+h"   # User hotkey to run the script (free text format)
SystemHotkey = "<Ctrl>+<Alt>+<End>"  # System hotkey to exit the script (free text format)
DasHotkey = "%t"        # Hotkey to pass into DAS ('+' for Shift, '^' for Ctrl, '%' for Alt)
SetRisk = 65            # Set dollar risk per trade here
CurrentPosition = ""    # Leave blank for MainFunction use
